...
class DDNS:
    def __init__(self, Id, Token):
        ...
        self.lang = "cn"
        self.headers = {
            "User-Agent": "ddns_for_raspberry_pi/0.1(heeeepin@gmail.com)"
        }
    ...
    def get_record_list(self):
        ...
        for i in self.domain_list:
            ...
            r = json.loads(
                requests.post(url=url, data=data, headers=self.headers).text)
            for j in r["records"]:
    ...
    def update(self):
        ...
        for i in record:
            ...
            r = json.loads(
                requests.post(url=url, data=d, headers=self.headers).text)
            if (r["status"]["code"] == "1"):
